...
const { isSolved, autoHint, nextStep, running, time, reset, input } = useState();
...
<template>
  <main class="h-screen w-screen flex items-center justify-center gap-8">
    <div class="space-y-2">
      <div class="text-gray-900 font-medium flex gap-4 min-h-6 w-full justify-between">
        <IconButton class="flex items-center gap-1" @click="onTogglePause"
          ><PlayIcon v-if="!running" class="size-5" /><PauseIcon v-else class="size-5" />{{
            time
          }}</IconButton
        >
        <div v-if="autoHint" class="font-light">{{ nextStep?.reason }}</div>
        <div class="flex items-center gap-2">
          <IconButton @click="reset"><ResetIcon class="size-5" /> </IconButton>
          <IconButton @click="onToggleLike">
            <HeartFilledIcon v-if="isLiked" class="size-5 text-blue-500" />
