  ...
  <main class="h-screen w-screen flex items-center justify-center gap-8">
    <div class="space-y-2">
      <div class="text-gray-900 font-medium flex gap-4 min-h-6 w-full justify-between items-center">
        <IconButton class="pl-0" @click="router.push('/')"><ChevronLeftIcon class="size-5" />Back</IconButton>
        <div v-if="autoHint" class="font-light text-sm">{{ nextStep?.reason }}</div>
        <div class="flex items-center gap-3">
          <IconButton class="font-light" @click="onTogglePause"
